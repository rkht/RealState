...
const AddRequirementInventoryScreen = ({ navigation }) => {
  ...
  const handleNextPress = () => {
    if (!selectedOption.listType || !selectedOption.serviceType || !selectedOption.propertyType) {
      ...
      setTimeout(() => {
        setLoading(false);
        console.log('Navigating to Next2 with selection:', selectedOption);
        navigation.navigate('Next2', { selectedOption });
      }, 1000);
    }
  };
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <TouchableOpacity style={styles.nextButton} onPress={handleNextPress} disabled={loading}>
        {loading ? <ActivityIndicator color="white" /> : <Text style={styles.nextButtonText}>Next (1/3)</Text>}
      </TouchableOpacity>
    </ScrollView>
  );
};
...
export default AddRequirementInventoryScreen;
