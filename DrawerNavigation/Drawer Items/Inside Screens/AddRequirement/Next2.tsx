...
const Next2 = ({ navigation, route }) => {
  ...
  const handleSave = () => {
    const formData = {
      city, zone, location, price, configuration, furnishedType, sqft, description, selectedOption
    };
    if (!city || !zone || !location || !price || !configuration || !furnishedType || !sqft || !description) {
      ...
      setTimeout(() => {
        setLoading(false);
        console.log('Navigating to Next3 with formData:', formData);
        navigation.navigate('Next3', formData);
      }, 1000);
    }
  };
      ...
      renderItem={() => (
        <View style={styles.container}>
          ...
          <TouchableOpacity style={styles.button} onPress={handleSave} disabled={loading}>
            {loading ? <ActivityIndicator color="#fff" /> : <Text style={styles.buttonText}>Next (2/3)</Text>}
          </TouchableOpacity>
        </View>
      )}
